...
/obj/item/integrated_circuit/interact(mob/user)
	...
	if((istype(src, /obj/item/integrated_circuit/manipulation/weapon_firing) && src.vars["installed_gun"] != null) || (istype(src, /obj/item/integrated_circuit/manipulation/ai) && src.vars["controlling"] != null) || (istype(src, /obj/item/integrated_circuit/manipulation/grenade) && src.vars["attached_grenade"] != null)) //Frontier add start
		HTML += "  |  <a href='?src=\ref[src];iremove=1'>\[Remove item\]</a>" //Frontier add end
	HTML += "<br>"
...
/obj/item/integrated_circuit/Topic(href, href_list, state = GLOB.physical_state)
	...
	if(href_list["pin"] && assembly)
	...
	else if(href_list["iremove"]) //Frontier add start
		if(istype(src,/obj/item/integrated_circuit/manipulation/weapon_firing))
			...
			B.detach_grenade()
		internal_examine(usr) //Frontier add end
